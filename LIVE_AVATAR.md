...
The live avatar is a memory radar with a persistent-file perimeter. The inner moving rings represent the current runtime memory snapshot. The three outer memory-signal rings represent delayed reports, L-T long-term facts, and active memory. A fourth, farther-out ring is made of file dots and represents the persistent files known to `window.JinFiles`. The center is the JIN accent light; it also feeds the scene tint.
...
| L-T blue dashes | Long-term facts that stay directly visible |
| Dim L-T dots | Ordinary report-covered L-T facts; archived, not deleted |
...
The important distinction is **representation versus activation**. A dim L-T dot or normal file dot still represents a real stored item. Loading, pinning, hovering, citing, or linking changes emphasis without changing that item's identity or angular slot.
...
Use this file for runtime rings, memory rings, the file-dot ring, radii, colors, speed, punctuation stripes, seeded geometry, center rendering, delayed/L-T/file link state, and avatar-level hover/reference/citation reactions.
...
| L-T archived/visible classification | `ui/static/js/runtime/runtime-lt-memory.js` | `getArchivedFactIdSet()`, `getVisibleFacts()`, `getFactsWithArchiveState()` |
| Long-term facts panel visibility | `ui/static/js/runtime/runtime.js` | `getVisibleLongTermMemoryFacts()` |
| Runtime/L-T/delayed/active/file row hover dispatch | `ui/static/js/runtime/runtime-memory-view.js` | row hover dispatch helpers and `data-avatar-memory-hover-id` |
...
4. Memory signal rings: delayed, L-T, active.
...
| `applyDelayedMemoryFactLinkGlow()` | delayed-report <-> L-T highlighting plus secondary-report state |
...
| `syncDelayedMemoryState()` | resyncs delayed state, L-T archive/link state, and file-link state |
...
- delayed, L-T, and active memory use their dedicated sync functions;
...
Each delayed/active item becomes one dash record. L-T always keeps one avatar record per fact, but archived L-T facts settle into dots instead of disappearing from the radar. The file ring uses one stable dot per persistent file.
...
| `arcTrimPixels` | Optional pixel trim before arc degrees are finalized; currently used by L-T |
...
The L-T ring shows all stored long-term facts. A directly exposed fact renders as a blue dash. A report-covered ordinary fact keeps the same avatar identity but renders as a dim dot.

There are two base visual states:

| L-T state | Meaning | Avatar |
|---|---|---|
...
Archived L-T records also pass:
...
Place: L-T branch inside `appendMemorySignalRing()`.

### L-T Archived Meaning

Archived does **not** mean deleted. It means the fact has been absorbed into delayed-memory report content and should not occupy a normal direct L-T line in context or the long-term panel.
...
7. Any directly pinned/loaded DM highlights all of its own linked L-T facts at Tier 1; those L-T facts move upward in the long-term panel.
...
### L-T Visual Edit Guide

| Desired change | File | Edit |
|---|---|---|
| Make archived dots dimmer/brighter | `runtime-avatar.js` | change archived opacity `0.26` |
| Make visible L-T facts brighter | `runtime-avatar.js` | change `0.52` |
| Change L-T color | `runtime-avatar.js` | `LT_MEMORY_RING_COLOR` |
| Move L-T ring | `runtime-avatar.js` | `MEMORY_RING_LAYOUT.lt.radius` |
| Make L-T dashes longer | `runtime-avatar.js` | `MEMORY_RING_LAYOUT.lt.arcRatio` or `maxArcDegrees` |
| Change archived dot transition | `runtime-avatar.css` | `.jin-avatar-memory-dash.is-memory-dot`, `jin-avatar-memory-absorb-dot` |
| Change archive semantics | `runtime-lt-memory.js` | `getArchivedFactIdSet()` |
| Hide archived facts from avatar entirely | `runtime-avatar.js` | use visible facts instead of `getFactsWithArchiveState()` in `getLTMemoryAvatarRecords()` |
...
| `is-delayed-memory-linked-hit` | Focused delayed report links to an L-T fact or file | strong cross-layer relation |
...
| `is-memory-archived` | L-T fact is structurally archived behind report content | hidden-from-direct-context marker |
| `is-memory-dot` | Archived L-T dot state | dash arc fades and dot remains |
...
- `ui/static/js/runtime/runtime-memory-view.js` — runtime/L-T/delayed/active/files rows plus delayed-report attachment chips/picker options;
...
For L-T facts, matching uses a strict identity tuple:
...
### Make archived L-T dots almost invisible
...
If the change is about **what a radar mark means**, edit the record collectors, delayed/L-T/file link logic, or L-T archive helper.
...
If an ordinary L-T fact belongs to a report, loading that report should change emphasis, not archive semantics. Anchor ids are the structural exception at the archive-id level; for merged facts, remember that `source_fact_ids` also participate in the final archived match.
