...
class MemoryAttentionTests(unittest.TestCase):
    def make_context(self, *, turns=None, facts=None):
        return SimpleNamespace(
            ...
            runtime_memory_attention_l4_focus_ids=[],
        )
    ...
    def test_l4_focus_is_one_to_three_and_prompt_only(self):
        facts = [
    ...
    def test_l4_does_not_pad_focus_with_unrelated_facts(self):
        facts = [
        ...
        self.assertEqual(context.runtime_memory_attention_l4_focus_ids, ["F2"])
        self.assertEqual([fact["id"] for fact in ranked], ["F2", "F3", "F1"])

    def test_l4_recent_facts_do_not_focus_themselves(self):
        facts = [
