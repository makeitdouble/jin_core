...
`CLEAN_TOOL_RESULTS` has an explicit field-local persistence rule. On successful cleanup, the browser writes the remaining `session_snapshot.tool_results` and monotonic `tool_result_sequence` inside the existing checkpoint and preserves checkpoint timestamp/lineage verbatim. `<CLEAN_TOOL_RESULTS>` removes all old results; `<CLEAN_TOOL_RESULTS: T1 >` removes only that modern block. Legacy blocks keep no ID and require full cleanup. Invalid IDs produce a failed bubble, session action, and readable failure tool result without removing existing data. During enrichment, the mere presence of `tool_results` (including `[]`) is authoritative, so old archived search/tool output cannot resurrect in a new tab. This exact-empty rule is intentionally **not** shared by `loaded_memory_ids` or `active_memory_records`; their empty browser collections still use the established archive fallback behavior.
