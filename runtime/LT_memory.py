...
from runtime.LT_memory_utils import (
    ...
    build_lt_merge_shard_scan_user_prompt,
...
LT_LOG_LEVEL = "L-T"
LT_DEFAULT_IDLE_SECONDS = 15
LT_CLOSED_TABS_INTERVAL_DIVISOR = 3
LT_MERGE_RETRY_BASE_SECONDS = 60
LT_MERGE_RETRY_MAX_SECONDS = 300
LT_MERGE_VALIDATION_RETRY_SECONDS = 50
LT_MERGE_POISON_DEFER_SECONDS = 300
...
def reset_lt_merge_recovery_state(
    context,
    *,
    keep_batch_limit: bool = False,
) -> None:
    if not keep_batch_limit:
        context.runtime_lt_merge_batch_limit = 0
        context.runtime_lt_merge_last_success_batch_limit = 0
        context.runtime_lt_merge_batch_locked = False
        context.runtime_lt_merge_existing_batch_mode = ""
        context.runtime_lt_merge_paused_signature = ""
        context.runtime_lt_merge_deferred_pending_until = {}
        context.runtime_lt_merge_single_retry_pending_ids = set()
        context.runtime_lt_merge_force_single_batch_once = False
    context.runtime_lt_merge_truncation_streak = 0
    context.runtime_lt_merge_retry_not_before = 0.0


def refresh_lt_merge_context_window_state(
    context,
    runtime_context_window: int,
) -> bool:
    """Release learned L-T batching limits when LM Studio's live n_ctx changes."""

    current = _positive_int(runtime_context_window)
    previous = _positive_int(
        getattr(
            context,
            "runtime_lt_merge_context_window_tokens",
            0,
        )
    )
    context.runtime_lt_merge_context_window_tokens = current
...
def record_lt_merge_success(
    ...
    *,
    ...
    locked = bool(
        getattr(
            context,
            "runtime_lt_merge_batch_locked",
            False,
        )
    )

    context.runtime_lt_merge_last_success_batch_limit = normalized_batch_count
    context.runtime_lt_merge_truncation_streak = 0
    context.runtime_lt_merge_retry_not_before = 0.0
    ...
    if locked:
        context.runtime_lt_merge_batch_limit = normalized_batch_count
        return {
    ...
    context.runtime_lt_merge_batch_limit = next_limit
    return {
...
def get_lt_merge_retry_backoff(context) -> dict | None:
    retry_not_before = float(
        getattr(
            context,
            "runtime_lt_merge_retry_not_before",
            0.0,
        )
        or 0.0
    )
...
def record_lt_merge_truncation(
    context,
    *,
    batch_count: int,
) -> dict:
    ...
    last_success = _positive_int(
        getattr(
            context,
            "runtime_lt_merge_last_success_batch_limit",
            0,
        )
    )
    ...
    streak = _positive_int(
        getattr(
            context,
            "runtime_lt_merge_truncation_streak",
            0,
        )
    ) + 1
    retry_after_seconds = min(
        LT_MERGE_RETRY_MAX_SECONDS,
        LT_MERGE_RETRY_BASE_SECONDS
        * (2 ** min(streak - 1, 4)),
    )

    context.runtime_lt_merge_batch_limit = next_batch_limit
    context.runtime_lt_merge_truncation_streak = streak
    context.runtime_lt_merge_retry_not_before = (
    ...
    return {
        ...
        "adaptive_batch_locked": bool(
            getattr(context, "runtime_lt_merge_batch_locked", False)
        ),
...
def clear_lt_merge_pending_recovery(
    context,
    pending_ids,
) -> None:
    deferred = dict(
        getattr(
            context,
            "runtime_lt_merge_deferred_pending_until",
            {},
        )
        or {}
    )
    ...
    context.runtime_lt_merge_deferred_pending_until = deferred

    single_retry_ids = set(
        getattr(
            context,
            "runtime_lt_merge_single_retry_pending_ids",
            set(),
        )
        or set()
    )
    ...
    context.runtime_lt_merge_single_retry_pending_ids = single_retry_ids


def record_lt_merge_shard_scan_failures(
    context,
    *,
    ...
    for pending_id in normalized_pending_ids:
        ...
        retry_after = (
            LT_MERGE_POISON_DEFER_SECONDS
            if already_isolated
            else LT_MERGE_VALIDATION_RETRY_SECONDS
        )
        if already_isolated:
...
def get_lt_merge_available_pending_queue(
    context,
    ...
    context.runtime_lt_merge_deferred_pending_until = next_deferred
    if active:
        return active, None
    if not waiting:
        return pending_queue, None
...
def record_lt_merge_validation_failure(
    context,
    *,
    pending_ids: list[str],
    batch_count: int,
    ...
    if normalized_batch_count > 1:
        context.runtime_lt_merge_force_single_batch_once = True
        context.runtime_lt_merge_retry_not_before = (
            time.monotonic() + LT_MERGE_VALIDATION_RETRY_SECONDS
        )
        return {
            "retry_behavior": (
            ...
            "next_batch_count": 1,
            "retry_after_seconds": LT_MERGE_VALIDATION_RETRY_SECONDS,
        }
    ...
    if pending_id:
        ...
        deferred[pending_id] = time.monotonic() + LT_MERGE_POISON_DEFER_SECONDS
        context.runtime_lt_merge_deferred_pending_until = deferred
    ...
    return {
        "retry_behavior": (
            ...
            "continue through L-T instead of deadlocking behind it."
        ),
        "deferred_pending_ids": [pending_id] if pending_id else [],
        "retry_after_seconds": LT_MERGE_POISON_DEFER_SECONDS,
    }


def build_lt_merge_validation_feedback(
    store: dict,
    ...
    return feedback_by_reason.get(
        reason,
        "Return corrected JSON that exactly satisfies the four-action L-T merge contract.",
    )


async def resolve_lt_request_limits(
    *,
    service_client,
    system_prompt: str,
    user_prompt: str,
...
def build_lt_truncation_details(
    response: dict,
    *,
    phase: str,
    ...
    request_meta = response.get("_jin_lt_request_meta", {})
    ...
    return (
        _positive_int(
            getattr(
                config,
                "LT_IDLE_SECONDS",
                LT_DEFAULT_IDLE_SECONDS,
            )
        )
        or LT_DEFAULT_IDLE_SECONDS
    )


def get_lt_scheduler_interval_seconds(
    *,
    tabs_open: bool,
) -> float:
    idle_seconds = float(get_lt_idle_seconds())
    if tabs_open:
        return idle_seconds
    return idle_seconds / float(LT_CLOSED_TABS_INTERVAL_DIVISOR)


def _lt_scheduler_wake_event(app_state):
    if app_state is None:
        return None
    return getattr(
        app_state,
        "lt_memory_scheduler_wake_event",
        None,
    )


def wake_lt_memory_server_scheduler(context) -> None:
    app_state = getattr(
        context,
        "runtime_lt_app_state",
        None,
    )
    wake_event = _lt_scheduler_wake_event(app_state)
    if wake_event is not None:
        wake_event.set()


def bind_lt_runtime_app_state(
    context,
    app_state,
) -> None:
    context.runtime_lt_app_state = app_state
    wake_lt_memory_server_scheduler(context)


def register_lt_websocket_connection(
    context,
    *,
    app_state,
    websocket,
    ...
    if connection_ids is None:
        connection_ids = set()
        app_state.lt_open_websocket_ids = connection_ids
    connection_ids.add(id(websocket))
    context.runtime_lt_websocket_connected = True
    wake_lt_memory_server_scheduler(context)


def unregister_lt_websocket_connection(
    context,
    *,
    app_state,
    websocket,
...
def _facts_memory_identity(
    ...
    return (
        normalize_lt_text(session_id),
        normalize_lt_key(key),
...
def _preserve_server_analyzed_facts_memory(
    ...
    for record in normalize_facts_memory_records(server_records):
        ...
        for key, field in record.get("signals", {}).items():
            if field.get("lt_status") != "analyzed":
                continue
            analyzed_by_identity[
                _facts_memory_identity(session_id, key, field)
            ] = str(field.get("lt_analyzed_at", "") or "")
...
def lt_memory_has_pending_work(context) -> bool:
...
def runtime_lt_file_store_enabled(context) -> bool:

    explicit = getattr(
        context,
        "runtime_lt_file_store_enabled",
        None,
    )
...
def get_runtime_lt_file_store_root(context):

    return getattr(
        context,
        "runtime_lt_file_store_root",
        None,
    )
...
def merge_with_runtime_lt_file_store(context, store) -> tuple[dict, bool]:
    ...
    file_store = load_runtime_lt_file_store(
        context,
    )
...
def set_runtime_lt_state(context, store) -> dict:

    merged, _changed = merge_with_runtime_lt_file_store(
        context,
        store,
    )
...
def get_context_delayed_memory_reports(
    ...
    # helper pure: L-T visibility must not depend on another prompt builder
...
def merge_lt_fact_report_id_maps(
    *maps,
) -> dict[str, list[str]]:
...
def refresh_runtime_lt_archived_fact_ids(
    context,
) -> set[str]:
...
def ensure_runtime_lt_state(context) -> dict:
    ...
    normalized_store = normalize_lt_store(
        getattr(context, "runtime_long_term_memory_store", None)
    )
    context.runtime_long_term_memory_store = set_runtime_lt_state(
        context,
        normalized_store,
    )
...
def apply_lt_memory_store_sync(context, raw_store) -> bool:
    ...
    incoming = normalize_lt_store(raw_store)
...
async def emit_lt_memory_update(context, *, change: dict | None = None) -> None:
    emit = getattr(getattr(context, "emitter", None), "emit", None)
    await safe_call(
        emit,
        {
            "type": "lt_memory_update",
...
def normalize_lt_fact_restore_report_refs(value) -> list[dict]:
...
def build_lt_deleted_fact_restore_meta(delayed_memory_change: dict) -> dict:
    refs = normalize_lt_fact_restore_report_refs({
...
def restore_delayed_memory_lt_fact_refs(
    context,
    *,
    fact_id: str,
    restore_meta: dict,
...
async def log_lt_skip_event(
    ...
    *,
    ...
    await log_memory_event(
        context,
        level=LT_LOG_LEVEL,
        message=f"L-T {message_phase} skipped: {display_reason}",
        details=json.dumps(
            result,
            ensure_ascii=False,
            indent=2,
...
def get_runtime_lt_explicit_edit_fact_ids(context) -> list[str]:
    current_turn_id = str(
        getattr(context, "runtime_current_turn_id", "")
        or ""
    ).strip()
    protected_turn_id = str(
        getattr(context, "runtime_lt_explicit_edit_turn_id", "")
        or ""
    ).strip()
    ...
    return normalize_long_term_fact_ids(
        list(
            getattr(context, "runtime_lt_explicit_edit_fact_ids", set())
            or []
        )
    )


def mark_runtime_lt_explicit_edit(
    context,
    fact_ids,
) -> None:
    current_turn_id = str(
    ...
    context.runtime_lt_explicit_edit_turn_id = current_turn_id
    context.runtime_lt_explicit_edit_fact_ids = (
        existing_ids | normalized_ids
    )


def protect_explicit_lt_edits_from_merge(
    operations: list[dict],
    protected_fact_ids,
    ...
    for operation in operations:
        ...
        if touches_protected:
            ...
            next_operations.append({
                ...
                "comment": "Protected by an explicit L-T edit in this turn.",
            })
            continue
...
def rebase_lt_merge_operations(
    *,
    ...
    base = clone_lt_store(base_store)
    current = clone_lt_store(current_store)
    ...
    for operation in rebased_operations:
        ...
        if action == "create":
            ...
            same_key = [
                ...
                if normalize_lt_key(fact.get("key")) == desired_signature[0]
            ]
            if same_key:
                exact_matches = [
                    fact
    ...
    rebased_store, rebased_change = apply_lt_merge_operations(
        current,
        rebased_operations,
...
async def log_lt_context_paused(
    context,
    *,
    ...
    context.runtime_lt_merge_paused_signature = signature
...
async def run_lt_merge_phase(*, context, service_client) -> dict:
    base_store = clone_lt_store(ensure_runtime_lt_state(context))
    ...
    retry_backoff = get_lt_merge_retry_backoff(context)
    if retry_backoff is not None:
        return retry_backoff

    pending_queue, deferred_backoff = get_lt_merge_available_pending_queue(
        context,
        pending_queue,
    )
    ...
    force_single_batch = bool(
        ...
        or getattr(
            context,
            "runtime_lt_merge_force_single_batch_once",
            False,
        )
    )
    ...
    async def execute_double_batch_plan(double_plan: dict) -> dict:
        ...
        if merge_mode == "full":
            ...
            merge_response = await ask_lt_model(
                context=context,
                service_client=service_client,
                label="L-T merge",
                system_prompt=system_prompt,
        ...
        scan_system_prompt = build_lt_merge_shard_scan_system_prompt()
        ...
        if is_runtime_memory_response_truncated(scan_response):
            ...
            details["shard"] = 1
            terminal = await log_lt_skip_event(
                context,
                phase="merge",
                message_phase="merge",
                reason="response_truncated",
        ...
        scan_payload = extract_lt_json_payload(
        ...
        scan_inspection = inspect_lt_merge_shard_scan(
            scan_payload,
            pending_ids=selected_pending_ids,
        ...
        if initial_invalid_ids:
            ...
            if is_runtime_memory_response_truncated(repair_response):
                ...
                repair_payload = extract_lt_json_payload(
                    extract_runtime_memory_text(repair_response)
                )
                repair_inspection = inspect_lt_merge_shard_scan(
                    repair_payload,
        ...
        while final_pending:
            ...
            final_previous_facts = collect_lt_shard_scan_referenced_facts(
                first_half,
                final_scan,
            )
            final_prompt = build_lt_merge_shard_finalize_user_prompt(
            ...
            minimum_finalize_required = (
                ...
                + estimate_lt_merge_response_tokens(final_pending)
        ...
        if not final_pending:
            terminal = await log_lt_context_paused(
                context,
    ...
    for provider_attempt in range(2):
        ...
        if not double_plan.get("fits"):
            return await log_lt_context_paused(
                context,
        ...
        context.runtime_lt_merge_paused_signature = ""
        context.runtime_lt_merge_existing_batch_mode = str(
        ...
        if (
            ...
            # on every idle tick.
            context.runtime_lt_merge_batch_limit = planned_batch_count
            context.runtime_lt_merge_batch_locked = True
    ...
    if execution is None:
        raise RuntimeError("L-T merge execution did not start")
    ...
    if (
        ...
        context.runtime_lt_merge_batch_locked = True
    if is_runtime_memory_response_truncated(response):
        truncation_details = build_lt_truncation_details(
            response,
            phase="merge",
    ...
    payload = extract_lt_json_payload(response_text)
    operations = []
    protected_pending_ids = []
    ...
    if payload is not None:
        operations = normalize_lt_merge_operations(payload)
        operations, protected_pending_ids = protect_explicit_lt_edits_from_merge(
            operations,
            protected_fact_ids,
        )
        next_store, merge_change = apply_lt_merge_operations(
            base_store,
            operations,
            pending_ids=pending_ids,
        )
    ...
    if not merge_change.get("valid"):
        feedback = build_lt_merge_validation_feedback(
            base_store,
            operations,
            initial_validation_reason,
        )
        ...
        if merge_mode == "halves":
            repair_prompt = build_lt_merge_shard_finalize_user_prompt(
        ...
        if repair_payload is None:
            ...
            repair_operations = normalize_lt_merge_operations(repair_payload)
            (
                repair_operations,
                repair_protected_pending_ids,
            ) = protect_explicit_lt_edits_from_merge(
                repair_operations,
                protected_fact_ids,
            )
            repair_next_store, repair_change = apply_lt_merge_operations(
                base_store,
                repair_operations,
                pending_ids=pending_ids,
            )
    ...
    if current_store != base_store:
        next_store, rebased_change = rebase_lt_merge_operations(
    ...
    batching_recovery = record_lt_merge_success(
        context,
        batch_count=len(pending_facts),
...
async def run_lt_jin_note(
    *,
    ...
    requested_action = infer_lt_jin_note_action(
    ...
    system_prompt = build_lt_jin_note_system_prompt()
    user_prompt = build_lt_jin_note_user_prompt(
    ...
    if not change.get("changed"):
        await log_memory_event(
            context,
            level=LT_LOG_LEVEL,
            message="L-T JIN note required no change",
            details=json.dumps(
                {
                    "selected_fact_ids": selected_fact_ids,
                    "message": message,
    ...
    *,
    ...
    try:
        ...
        if user_idle_seconds is not None:
            ...
            if normalized_idle_seconds < get_lt_idle_seconds():
        ...
        if pending_fields:
            extraction_result = await run_lt_extraction_phase(
                context=context,
                service_client=service_client,
        ...
        if ensure_runtime_lt_state(context).get("pending_facts"):
            merge_result = await run_lt_merge_phase(
                context=context,
                service_client=service_client,
            )
        ...
        await log_memory_event(
            context,
            level=LT_LOG_LEVEL,
            message="L-T update failed",
            details=build_memory_failure_details(
                stage="L-T memory update",
                error=error,
...
def _lt_server_contexts(app_state) -> list:
    store = getattr(
        app_state,
...
def _lt_server_context(app_state):
    ...
    context = max(
        ...
        key=lambda candidate: float(
            getattr(candidate, "runtime_lt_profile_sync_at", 0.0)
            or 0.0
        ),
    )
    app_state.lt_runtime_context = context
...
async def run_lt_memory_server_scheduler(app_state) -> None:
    wake_event = _lt_scheduler_wake_event(app_state)
    if wake_event is None:
        wake_event = asyncio.Event()
        app_state.lt_memory_scheduler_wake_event = wake_event
    ...
    while True:
        ...
        if _lt_server_foreground_busy(app_state):
...
def start_lt_memory_server_scheduler(app_state) -> asyncio.Task:
    existing = getattr(
        app_state,
        "lt_memory_scheduler_task",
        None,
    )
    if existing is not None and not existing.done():
        return existing

    app_state.lt_open_websocket_ids = set()
    app_state.lt_facts_memory_records = []
    app_state.lt_last_user_activity_at = 0.0
    app_state.lt_memory_scheduler_wake_event = asyncio.Event()
    task = asyncio.create_task(
        run_lt_memory_server_scheduler(app_state)
    )
    app_state.lt_memory_scheduler_task = task
    return task


async def stop_lt_memory_server_scheduler(app_state) -> None:
    task = getattr(
        app_state,
        "lt_memory_scheduler_task",
        None,
    ...
    with contextlib.suppress(
        ...
        await task
    app_state.lt_memory_scheduler_task = None


def lt_fact_matches_archived_ids(
    fact: dict,
...
def _get_lt_fact_anchor_report_ids(
    fact: dict,
...
def build_runtime_lt_memory_context(*, context, fact_ids=None, user_input: str = "") -> str:
    ...
    report_ids_by_fact_id = merge_lt_fact_report_id_maps(
    ...
    try:
        from runtime.memory_attention import rank_lt_facts_for_context

        active_facts = rank_lt_facts_for_context(
            active_facts,
            context=context,
    ...
    for fact in active_facts:
        ...
        if not fact_id:
            continue
        report_ids = _get_lt_fact_anchor_report_ids(
            fact,
...
async def delete_lt_memory_fact(context, fact_id: str) -> bool:
    current_store = ensure_runtime_lt_state(context)
    ...
    store, changed = delete_lt_fact_from_store(
        current_store,
        target_id,
    )
    ...
    restore_meta = build_lt_deleted_fact_restore_meta(
...
async def restore_lt_memory_fact(context, fact) -> bool:
    restore_meta = (
        fact.get("_restore_meta", {})
        if isinstance(fact, dict)
        else {}
    )
    store, changed = restore_lt_fact_to_store(
        ensure_runtime_lt_state(context),
        fact,
    )
    if not changed:
    ...
    delayed_memory_change = restore_delayed_memory_lt_fact_refs(
        context,
