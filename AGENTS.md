...
- Physical archive deletion invalidates its normal-browser bootstrap replica. Select the newest surviving real USER archive even when older than localStorage; never resurrect deleted dialogue/FRAME from browser timestamps. Startup-only logs stay in RAM until a real USER send.
