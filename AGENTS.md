...
- Skill loading is model-facing `<LOAD_SKILL_CONTEXT> skill_name </LOAD_SKILL_CONTEXT>` with exactly one skill per block. `LOAD_SKILL` is the internal runtime action name; old `LOAD_SKILL`/`LOAD_SKILLS` tags do not inherit executable alias status from the renamed public marker.
...
- Current Active create/update model boundary is structured JSON but not one shared flat shape. `SAVE_ACTIVE_MEMORY` keeps `conditions` plus optional custom fields at the JSON root. `UPDATE_ACTIVE_MEMORY` uses `active_memory_id` plus a mandatory `fields_to_update` object, even for one field. Plain prose such as a trailing `(field: value)` remains conditions, not schema. Legacy flat/nested/line-based update payloads and self-closing UPDATE attributes may be accepted locally for compatibility; do not advertise them as preferred syntax.
