(function () {
  ...
  function getContextPressureRatio() {
    ...
    const rawPercent = window
      .getComputedStyle(root)
      .getPropertyValue("--jin-context-pressure-percent");
    ...
    return clamp(numericPercent / 100, 0, 1);
  }

  function buildRayOpacityProfile(pressureRatio, localStrength = 1) {
    ...
    return {
      base: 0,
      soft: peakOpacity * 0.34,
      mid: peakOpacity * 0.68,
      peak: peakOpacity,
    };
  }
  ...
  function appendStaticScaffold(svg, overallColor, currentCenterColor, diffPercent, random, avatarLayout) {
    ...
    const pressureRatio = getContextPressureRatio();
    const rayCount = 16;
    const activeRayCount =
      Math.max(3, Math.min(7, 3 + Math.round(pressureRatio * 4)));
    ...
    scaffoldRadii.forEach((radius, index) => {
      scaffoldContent.appendChild(createSvgElement("circle", {
        class: "jin-avatar-scaffold-ring",
        cx: CENTER,
        cy: CENTER,
        r: radius,
        ...
        style: `--jin-avatar-ring-fallback:${index % 2 ? overallColor : ACCENT_RING_COLOR}`,
      }));
    });
    ...
    for (let index = 0; index < rayCount; index += 1) {
      ...
      const strengthSeed = Math.pow(random(), activeRay ? 0.72 : 1.9);
      const localStrength =
        activeRay
          ? 0.60 + strengthSeed * 0.40
          : 0.12 + strengthSeed * 0.32;
      const opacityProfile =
        buildRayOpacityProfile(pressureRatio, localStrength);
      ...
      const rayColor =
        mixColors(
          ...
          "#081018",
          activeRay
            ? 0.40 + (1 - pressureRatio) * 0.10
            : 0.52 + (1 - pressureRatio) * 0.10
        );
      ...
      scaffoldContent.appendChild(createSvgElement("line", {
        ...
        "stroke-opacity": "1",
        style: [
          `--jin-avatar-ray-color:${rayColor}`,
          `--jin-avatar-ray-base-opacity:${opacityProfile.base.toFixed(3)}`,
          `--jin-avatar-ray-soft-opacity:${opacityProfile.soft.toFixed(3)}`,
          `--jin-avatar-ray-mid-opacity:${opacityProfile.mid.toFixed(3)}`,
          `--jin-avatar-ray-peak-opacity:${opacityProfile.peak.toFixed(3)}`,
  ...
  function appendCenter(svg, overallColor, currentCenterColor) {
    ...
    [24, 31, 39].forEach((radius, index) => {
      center.appendChild(createSvgElement("circle", {
        ...
        style: `--jin-avatar-ring-fallback:${index === 1 ? "#f4f7f5" : overallColor}`,
      }));
    });
