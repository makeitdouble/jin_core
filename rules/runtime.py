...
INTERNAL_ACTION_APPEND_SKILLS_MARKER = "<INTERNAL_ACTION_APPEND_SKILLS: name1, name2, name3 >"
INTERNAL_ACTION_REMOVE_SKILLS_MARKER = "<INTERNAL_ACTION_REMOVE_SKILLS: name1, name2, name3 >"
...
#    "Emit marker ONLY, DO NOT add any other in the same output.\n"
    "Skip notifing user about emitting a markers during a task process.\n"
    ...
    "never write full code in reasoning; "
    "never output file content in your final answer, use filename and file path instead; "
    "never qoute or explain code in your final answer unless user explicitly asking.\n"
    
    "SESSION_ACTIONS_HISTORY is the authoritative progress ledger for LATEST_USER_REQUEST.\n"
    "If no runtime action is needed, output the user-facing final result or usual response.\n"
    "Each action in SESSION_ACTIONS_HISTORY means that this runtime action has already been executed for one concrete part/subgoal of the latest user request.\n"
    "If SESSION_ACTIONS_HISTORY shows action was done 0s ago, it means this action part of the current workflow is already completed, even if it was not the first part of the task.\n"
    "Validate and sync current action ( 0s ago ) with current trusted state and decide what remaining subgoal is still unresolved.\n"
    "Never repeat action that just appeared and indicates ( 0s ago ) even if conditions mandate to do it.\n"
    "Do not count repeated user request as repeated required actions; count only unresolved task subgoals.\n"
    "Before emitting any runtime marker, ask: which specific unresolved part of LATEST_USER_REQUEST will this action complete?\n"
    "If an action was just completed and no distinct unresolved subgoal requires the same action again, do not repeat it; "
    "move to the next required action or provide final answer the user.\n"
    "When the required internal actions are already completed - you must request done "
    "and immediately stop and send the final user-facing completion response for LATEST_USER_REQUEST.\n"
    "You must sync with <SESSION_ACTIONS_HISTORY> to track your latest actions!\n"
    "If system indicates required actions for achieving LATEST_USER_REQUEST are made, you can produce the final user-facing usual response.\n"
    "Runtime follow-up tick is service message and do not require additional text in the answer.\n"
    ...
    "On a runtime follow-up tick resume or finish the workflow derived from LATEST_USER_REQUEST and SESSION_ACTIONS_HISTORY.\n"
)
...
APPEND_REMOVE_SKILL_RULES = (
    ...
    "For multiple appending use following markers:"
    ...
    "If the relevant skill is present in APPENDED_SKILLS, do not append it again, continue or notify user.\n"
)
