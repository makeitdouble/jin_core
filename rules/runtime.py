...
SKILL_ROUTING_RULES = (
    "SKILL ROUTING:\n"
    "NEVER reiterate same action from last position inside <CURRENT_APPENDED_SKILLS>.\n"
    "If user ask to add or remove skill YOU MUST CHECK <CURRENT_APPENDED_SKILLS> AND <SESSION_ACTIONS_HISTORY> BEFORE appending ANY skill.\n"
...
#    "- A skill is only a block of textual instructions appended to the prompt context.\n"
#    "- A skill is not a callable tool, not a runtime action, and not an executable marker.\n"
#    "- To use an appended skill, silently apply its text instructions to the current response.\n"
#    "- Do not say that you will invoke, run, call, execute, or trigger a skill.\n"
#    "- Do not create internal action marker names from skill names.\n"
#    "- Only emit runtime markers that are explicitly listed as available runtime actions.\n"
#    "- If the needed skill is already listed in <CURRENT_APPENDED_SKILLS>, do not list skills, do not append it again, and do not emit any skill-related action.\n"
#    "- When the user asks to use an already appended skill, produce the final answer directly according to that skill.\n"
#    "- If no real runtime action is needed, output only the user-facing final result.\n\n"
)
...
RUNTIME_ACTIONS_RULES = (
    ...
    "On a runtime follow-up tick, ignore the literal USER PROMPT text and resume/finish the workflow derived from LATEST_USER_REQUEST.\n"
    "Do not require <CURRENT_RUNTIME_TODO_LIST> for simple one-step tasks.\n"
    "On every runtime follow-up tick, compare LATEST_USER_REQUEST with <SESSION_ACTIONS_HISTORY> and ask: what remains to satisfy the latest request?\n"
    "If a required action is already present in <SESSION_ACTIONS_HISTORY>, do not emit it again.\n"
    "If the latest request still requires another internal action, emit the next correct marker.\n"
    "If all required actions for LATEST_USER_REQUEST are already completed, produce the final user-facing completion response.\n"
    "Never answer to a runtime follow-up tick with idle, waiting, clarification, or generic conversational text.\n"
    "If there is no <CURRENT_RUNTIME_TODO_LIST>, still finish the latest request from LATEST_USER_REQUEST and <SESSION_ACTIONS_HISTORY>.\n"
